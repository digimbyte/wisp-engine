...
private:
    ...
    void renderConfirmation() {
        ...
        gfx->drawText(("Auto-cancel in " + String(remaining) + "s").c_str(), SCREEN_WIDTH / 2, SCREEN_HEIGHT - 25, true);
    }
    ...
    void renderCPUFrequencyConfig() {
        ...
        gfx->drawText(("Est. Power: " + String(powerEstimates[settings.cpuFrequency])).c_str(), 
    ...
    void renderSystemStatus() {
        ...
        // CPU info
        gfx->drawText(("CPU: " + String(getCpuFrequencyMhz()) + "MHz").c_str(), 5, SCREEN_HEIGHT - 15, false);
        
        // Memory info
        String memInfo = String(ESP.getFreeHeap() / 1024) + "KB Free";
    ...
    String getValueText(SystemMenuState item) {
        switch (item) {
            ...
            case STORAGE_INFO:
                return String(SPIFFS.usedBytes() / 1024) + "/" + String(SPIFFS.totalBytes() / 1024) + "KB";
