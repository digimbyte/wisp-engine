...
public:
    ...
    void init(GameLoopManager* manager) {
        ...
        WISP_DEBUG_INFO("WISP", "=== Frame Rate Demo App Started ===");
        WISP_DEBUG_INFO("WISP", "This app demonstrates:");
        WISP_DEBUG_INFO("WISP", "- Target 30 FPS with minimum 15 FPS");
        WISP_DEBUG_INFO("WISP", "- Adaptive frame rate scaling enabled");
        WISP_DEBUG_INFO("WISP", "- Performance monitoring every 5 seconds");
        WISP_DEBUG_INFO("WISP", "- Simple moving entity for load testing");
    }
    ...
    void printAppPerformanceReport() {
        WISP_DEBUG_INFO("WISP", "=== App Performance Report ===");
        
        // Frame rate information
        WISP_DEBUG_INFO("WISP", "Current FPS available");
        WISP_DEBUG_INFO("WISP", "Target FPS configured");
        WISP_DEBUG_INFO("WISP", "Frame drop percentage tracked");
        
        // Memory usage (if available)
        WISP_DEBUG_INFO("WISP", "Free heap memory available");
        
        // Entity status
        GameEntity* player = gameLoopManager->getEntity(playerEntity);
        if (player) {
            WISP_DEBUG_INFO("WISP", "Player position tracked");
        }
        
        WISP_DEBUG_INFO("WISP", "============================");
    }
    
    // Test different frame rate configurations
    void testFrameRateScaling() {
        WISP_DEBUG_INFO("WISP", "Testing frame rate scaling...");
        
        // Test high performance mode
        WISP_DEBUG_INFO("WISP", "Setting 60 FPS...");
        gameLoopManager->setTargetFrameRate(FRAMERATE_60FPS);
        vTaskDelay(pdMS_TO_TICKS(3000));
        
        // Test balanced mode
        WISP_DEBUG_INFO("WISP", "Setting 30 FPS...");
        gameLoopManager->setTargetFrameRate(FRAMERATE_30FPS);
        vTaskDelay(pdMS_TO_TICKS(3000));
        
        // Test power saving mode
        WISP_DEBUG_INFO("WISP", "Setting 15 FPS...");
        gameLoopManager->setTargetFrameRate(FRAMERATE_15FPS);
        vTaskDelay(pdMS_TO_TICKS(3000));
        
        // Test ultra power saving
        WISP_DEBUG_INFO("WISP", "Setting 8 FPS...");
        gameLoopManager->setTargetFrameRate(FRAMERATE_8FPS);
        vTaskDelay(pdMS_TO_TICKS(3000));
        
        // Back to default
        WISP_DEBUG_INFO("WISP", "Back to 30 FPS...");
        gameLoopManager->setTargetFrameRate(FRAMERATE_30FPS);
    }
    ...
    void toggleAdaptiveScaling() {
        ...
        WISP_DEBUG_INFO("WISP", "Adaptive frame rate scaling status");
    }
};
