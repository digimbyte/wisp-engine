...
enum AppStage {
    ...
    STAGE_LOGIC_UPDATE,        // App/game logic execution (C++ applications)
...
class AppLoop {
public:
    // Core systems (injected references, not owned)
    GraphicsEngine* graphics;
    PhysicsEngine* physics;
    AudioEngine* audio;
    InputController* input;
    
    // App callback interface (injected)
    void* appManager;  // Forward declaration to avoid circular dependency
    ...
    void init(GraphicsEngine* gfx, PhysicsEngine* phys, AudioEngine* aud, InputController* inp) {
        ...
        audio = aud;
        input = inp;
        appManager = nullptr;  // Will be set by AppLoopManager
    ...
    void update() {
        ...
        // Execute all game stages in order
        for (int stage = 0; stage < STAGE_COUNT; stage++) {
            currentStage = (GameStage)stage;
    ...
    void executeStage(GameStage stage) {
        switch (stage) {
    ...
    void stageLogicUpdate() {
        // Call app manager update if available
        if (appManager) {
            // Forward declaration - actual call handled by AppLoopManager
            // This keeps the app loop decoupled from app management
        }
    }
    ...
    GameEntity* getEntity(uint16_t entityId) {
        for (auto& entity : entities) {
...
private:
    ...
    void processTriggerEvent(const PhysicsEvent& event) {
        // Forward events to app manager if available
        if (appManager) {
            // Forward declaration - actual call handled by AppLoopManager
            // This keeps the app loop decoupled from app management
        }
