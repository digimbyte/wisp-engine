...
inline bool WispBootloader::init() {
    Serial.println("Initializing Wisp Bootloader...");
    ...
    Serial.println("Wisp Bootloader initialized");
    return true;
}
...
inline void WispBootloader::handleEngineInit() {
    Serial.println("Initializing engine systems...");
    ...
    if (graphics->loadEnhancedLUT(lut_palette_lut)) {
        Serial.println("WISP Bootloader: Enhanced LUT system loaded");
        ...
        Serial.println("WISP Bootloader: Using legacy LUT system");
    }
    ...
    Serial.println("Sprite layer system initialized with 8 layers");
...
inline void WispBootloader::handleAppDiscovery() {
    Serial.println("Scanning for applications...");
...
inline void WispBootloader::handleMenuCheck() {
    ...
    if (selectedAppIndex >= 0 && selectedAppIndex < (int)availableApps.size()) {
        if (availableApps[selectedAppIndex].autoStart) {
            Serial.println("Auto-starting application...");
...
inline void WispBootloader::advanceStage() {
    ...
    Serial.print("Boot stage advanced to: ");
    Serial.println((int)currentStage);
}
    ...
    while (file) {
        ...
        if (fileName.endsWith(".wisp") && !file.isDirectory()) {
            ...
            if (loadAppInfo(fullPath, appInfo)) {
                ...
                availableApps.push_back(appInfo);
                Serial.print("Added app (basic info): ");
                Serial.println(appInfo.name);
            }
        }
...
inline bool WispBootloader::loadAppInfo(const String& appPath, AppInfo& info) {
    // For now, just extract basic info from filename
    // In the future, this could read metadata from the .wisp file header
    
    File wispFile = SD.open(appPath);
    if (!wispFile) {
        Serial.print("Failed to open .wisp file: ");
...
inline void WispBootloader::handleAppLaunch() {
    ...
    if (selectedAppIndex >= 0 && selectedAppIndex < (int)availableApps.size()) {
        if (launchApp(selectedAppIndex)) {
            Serial.println("App launch completed");
        } else {
            Serial.println("App launch failed, returning to menu");
        }
    } else {
        Serial.println("No valid app selected, returning to menu");
        currentStage = BOOT_MENU_DISPLAY;
        menuActive = true;
    }
}
