...
// Define memory pool sizes for DocDatabase based on hardware capabilities
#define WISP_DB_RTC_SRAM_SIZE_BYTES  (ESP32_S3_RTC_MEMORY_KB * 1024)        // 16KB RTC memory pool
#define WISP_DB_PSRAM_MAX_BYTES      (1024 * 1024)                          // Up to 1MB PSRAM pool for large databases
#define WISP_DB_SRAM_MAX_BYTES       (128 * 1024)                           // Up to 128KB SRAM pool for speed
#define WISP_DB_DEFAULT_SIZE_BYTES   WISP_DB_RTC_SRAM_SIZE_BYTES             // Default to RTC for persistence
...
// === BLUETOOTH TYPE CONFIGURATION ===
// ESP32-S3 modules can support both Bluetooth Classic (BTE) and Low Energy (BLE)
// Configuration depends on the specific module variant and enabled features
#ifndef BLUETOOTH_TYPE_BLE
    #define BLUETOOTH_TYPE_BLE      1              // Bluetooth Low Energy support (default enabled)
#endif
#ifndef BLUETOOTH_TYPE_BTE  
    #define BLUETOOTH_TYPE_BTE      1              // Bluetooth Classic (Traditional) support (default enabled)
#endif
#define BLUETOOTH_TYPE_NULL         0              // No Bluetooth disabled

// Override for basic ESP32-S3 modules without wireless capabilities:
// In platformio.ini, add: -DBLUETOOTH_TYPE_BLE=0 -DBLUETOOTH_TYPE_BTE=0 -DBLUETOOTH_TYPE_NULL=1
...
// Most ESP32-S3 boards have addressable RGB LEDs (varies by board)
#ifndef RGB_LED_PIN
    #define RGB_LED_PIN             38            // Common GPIO for addressable LED (user-configurable)
#endif
#ifndef RGB_LED_TYPE
    #define RGB_LED_TYPE            "WS2812"      // Addressable RGB LED type
#endif
#ifndef RGB_LED_COUNT
    #define RGB_LED_COUNT           1             // Single RGB LED (user-configurable)
#endif
#define RGB_LED_POWER_PIN           -1            // No separate power pin
#define RGB_LED_PANEL               "NONE"        // No special panel
